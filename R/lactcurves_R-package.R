...
start=list(a=19.8,b=-1.65),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICMM=BIC(MM)
mMM=MM$call
RSSMM=sum(residuals(MM)^2, na.rm=T)
TSSMM=sum((trait-mean(trait))^2, na.rm=T)
PSSMM=sum((predict(MM)-mean(predict(MM), na.rm=T))^2, na.rm=T)
FMM=if(TSSMM>PSSMM){TSSMM/PSSMM} else {PSSMM/TSSMM}
RSDMM=sqrt(RSSMM/(n-coefMM))
RSMM=residuals(MM)
RS1MM=as.numeric(c(residuals(MM)[-1],"NA"))
DWMM=sum((RSMM-RS1MM)^2, na.rm=T)/sum(RSMM^2, na.rm=T)
	}else
...
start=list(a=-10,b=-1.4),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICMMR=BIC(MMR)
mMMR=MMR$call
RSSMMR=sum(residuals(MMR)^2, na.rm=T)
TSSMMR=sum((trait-mean(trait,na.rm=T))^2, na.rm=T)
PSSMMR=sum((predict(MMR)-mean(predict(MMR), na.rm=T))^2, na.rm=T)
FMMR=if(TSSMMR>PSSMMR){TSSMMR/PSSMMR} else {PSSMMR/TSSMMR}
RSDMMR=sqrt(RSSMMR/(n-coefMMR))
RSMMR=residuals(MMR)
RS1MMR=as.numeric(c(residuals(MMR)[-1],"NA"))
DWMMR=sum((RSMMR-RS1MMR)^2, na.rm=T)/sum(RSMMR^2, na.rm=T)
	}else
...
start=list(a=-0.06608,b=317.49, c=-328.06, d=-0.027),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICMME=BIC(MME)
mMME=MME$call
RSSMME=sum(residuals(MME)^2, na.rm=T)
TSSMME=sum((trait-mean(trait))^2, na.rm=T)
PSSMME=sum((predict(MME)-mean(predict(MME), na.rm=T))^2, na.rm=T)
FMME=if(TSSMME>PSSMME){TSSMME/PSSMME} else {PSSMME/TSSMME}
RSDMME=sqrt(RSSMME/(n-coefMME))
RSMME=residuals(MME)
RS1MME=as.numeric(c(residuals(MME)[-1],"NA"))
DWMME=sum((RSMME-RS1MME)^2, na.rm=T)/sum(RSMME^2, na.rm=T)
	}else
...
start=list(a=25.6,b=0.0015),control=list(maxiter=100,warnOnly=T)),silent=T)
...
RSSbrody23=sum(residuals(brody23)^2, na.rm=T)
TSSbrody23=sum((trait-mean(trait,na.rm=T))^2, na.rm=T)
PSSbrody23=sum((predict(brody23)-mean(predict(brody23),na.rm=T))^2, na.rm=T)
...
DWbrody23=sum((RSbrody23-RS1brody23)^2, na.rm=T)/sum(RSbrody23^2, na.rm=T)
	}else
...
start=list(a=26.127,b=0.0017,c=0.2575),control=list(maxiter=100,warnOnly=T)),silent=T)
...
RSSbrody24=sum(residuals(brody24)^2, na.rm=T)
TSSbrody24=sum((trait-mean(trait))^2, na.rm=T)
PSSbrody24=sum((predict(brody24)-mean(predict(brody24), na.rm=T))^2, na.rm=T)
...
DWbrody24=sum((RSbrody24-RS1brody24)^2, na.rm=T)/sum(RSbrody24^2, na.rm=T)
	}else
...
start=list(a=3, b=58),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICSCH=BIC(SCH)
mSCH=SCH$call
RSSSCH=sum(residuals(SCH)^2, na.rm=T)
TSSSCH=sum((trait-mean(trait))^2, na.rm=T)
PSSSCH=sum((predict(SCH)-mean(predict(SCH), na.rm=T))^2, na.rm=T)
FSCH=if(TSSSCH>PSSSCH){TSSSCH/PSSSCH} else {PSSSCH/TSSSCH}
RSDSCH=sqrt(RSSSCH/(n-coefSCH))
RSSCH=residuals(SCH)
RS1SCH=as.numeric(c(residuals(SCH)[-1],"NA"))
DWSCH=sum((RSSCH-RS1SCH)^2, na.rm=T)/sum(RSSCH^2, na.rm=T)
	}else
...
start=list(a=264, b=-2.6),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICSCHL=BIC(SCHL)
mSCHL=SCHL$call
RSSSCHL=sum(residuals(SCHL)^2, na.rm=T)
TSSSCHL=sum((trait-mean(trait))^2, na.rm=T)
PSSSCHL=sum((predict(SCHL)-mean(predict(SCHL), na.rm=T))^2, na.rm=T)
...
DWSCHL=sum((RSSCHL-RS1SCHL)^2, na.rm=T)/sum(RSSCHL^2, na.rm=T)
	}else
...
start=list(a=25, b=-0.0008, c=0.000002),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICPBE=BIC(PBE)
mPBE=PBE$call
RSSPBE=sum(residuals(PBE)^2, na.rm=T)
TSSPBE=sum((trait-mean(trait))^2, na.rm=T)
PSSPBE=sum((predict(PBE)-mean(predict(PBE), na.rm=T))^2, na.rm=T)
FPBE=if(TSSPBE>PSSPBE){TSSPBE/PSSPBE} else {PSSPBE/TSSPBE}
RSDPBE=sqrt(RSSPBE/(n-coefPBE))
RSPBE=residuals(PBE)
RS1PBE=as.numeric(c(residuals(PBE)[-1],"NA"))
DWPBE=sum((RSPBE-RS1PBE)^2, na.rm=T)/sum(RSPBE^2, na.rm=T)
	}else
...
start=list(a=20.5,b=0.07,c=0.002),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICwood=BIC(wood)
mwood=wood$call
RSSwood=sum(residuals(wood)^2, na.rm=T)
TSSwood=sum((trait-mean(trait))^2, na.rm=T)
PSSwood=sum((predict(wood)-mean(predict(wood), na.rm=T))^2, na.rm=T)
...
DWwood=sum((RSwood-RS1wood)^2, na.rm=T)/sum(RSwood^2, na.rm=T)
	}else
...
start=list(a=20.5, b=30.7, c=0.002),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICDHA=BIC(DHA)
mDHA=DHA$call
RSSDHA=sum(residuals(DHA)^2, na.rm=T)
TSSDHA=sum((trait-mean(trait))^2, na.rm=T)
PSSDHA=sum((predict(DHA)-mean(predict(DHA), na.rm=T))^2, na.rm=T)
FDHA=if(TSSDHA>PSSDHA){TSSDHA/PSSDHA} else {PSSDHA/TSSDHA}
RSDDHA=sqrt(RSSDHA/(n-coefDHA))
RSDHA=residuals(DHA)
RS1DHA=as.numeric(c(residuals(DHA)[-1],"NA"))
DWDHA=sum((RSDHA-RS1DHA)^2, na.rm=T)/sum(RSDHA^2, na.rm=T)
	}else
...
BICCB=BIC(CB)
mCB=CB$call
RSSCB=sum(residuals(CB)^2, na.rm=T)
TSSCB=sum((trait-mean(trait))^2, na.rm=T)
PSSCB=sum((predict(CB)-mean(predict(CB), na.rm=T))^2, na.rm=T)
FCB=if(TSSCB>PSSCB){TSSCB/PSSCB} else {PSSCB/TSSCB}
RSDCB=sqrt(RSSCB/(n-coefCB))
RSCB=residuals(CB)
RS1CB=as.numeric(c(residuals(CB)[-1],"NA"))
DWCB=sum((RSCB-RS1CB)^2, na.rm=T)/sum(RSCB^2, na.rm=T)
	}else
...
start=list(a=25, b=-0.02, c=-0.000015),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICQP=BIC(QP)
mQP=QP$call
RSSQP=sum(residuals(QP)^2, na.rm=T)
TSSQP=sum((trait-mean(trait))^2, na.rm=T)
PSSQP=sum((predict(QP)-mean(predict(QP), na.rm=T))^2, na.rm=T)
FQP=if(TSSQP>PSSQP){TSSQP/PSSQP} else {PSSQP/TSSQP}
RSDQP=sqrt(RSSQP/(n-coefQP))
RSQP=residuals(QP)
RS1QP=as.numeric(c(residuals(QP)[-1],"NA"))
DWQP=sum((RSQP-RS1QP)^2, na.rm=T)/sum(RSQP^2, na.rm=T)
	}else
...
start=list(a=25, b=0.03, c=0.28),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICCLD=BIC(CLD)
mCLD=CLD$call
RSSCLD=sum(residuals(CLD)^2, na.rm=T)
TSSCLD=sum((trait-mean(trait))^2, na.rm=T)
PSSCLD=sum((predict(CLD)-mean(predict(CLD), na.rm=T))^2, na.rm=T)
FCLD=if(TSSCLD>PSSCLD){TSSCLD/PSSCLD} else {PSSCLD/TSSCLD}
RSDCLD=sqrt(RSSCLD/(n-coefCLD))
RSCLD=residuals(CLD)
RS1CLD=as.numeric(c(residuals(CLD)[-1],"NA"))
DWCLD=sum((RSCLD-RS1CLD)^2, na.rm=T)/sum(RSCLD^2, na.rm=T)
	}else
...
start=list(a=24.7, b=-0.0098, c=0.0033),control=list(maxiter=100,warnOnly=T)),silent=T)

options(warn=-1)
#if(exists("PapBo1")=="TRUE"){
if(class(PapBo1)=="nls"){
...
mPapBo1=PapBo1$call
RSSPapBo1=sum(residuals(PapBo1)^2, na.rm=T)
TSSPapBo1=sum((trait-mean(trait))^2, na.rm=T)
PSSPapBo1=sum((predict(PapBo1)-mean(predict(PapBo1), na.rm=T))^2, na.rm=T)
...
DWPapBo1=sum((RSPapBo1-RS1PapBo1)^2, na.rm=T)/sum(RSPapBo1^2, na.rm=T)
	}else
...
start=list(a=23.896, b=0.398, c=0.0034),control=list(maxiter=100,warnOnly=T)),silent=T)
...
mPapBo2=PapBo2$call
RSSPapBo2=sum(residuals(PapBo2)^2, na.rm=T)
TSSPapBo2=sum((trait-mean(trait))^2, na.rm=T)
PSSPapBo2=sum((predict(PapBo2)-mean(predict(PapBo2), na.rm=T))^2, na.rm=T)
...
DWPapBo2=sum((RSPapBo2-RS1PapBo2)^2, na.rm=T)/sum(RSPapBo2^2, na.rm=T)
	}else
...
start=list(a=15.27, b=2.587, c=0.00346),control=list(maxiter=100,warnOnly=T)),silent=T)
...
mPapBo3=PapBo3$call
RSSPapBo3=sum(residuals(PapBo3)^2, na.rm=T)
TSSPapBo3=sum((trait-mean(trait))^2, na.rm=T)
PSSPapBo3=sum((predict(PapBo3)-mean(predict(PapBo3), na.rm=T))^2, na.rm=T)
...
DWPapBo3=sum((RSPapBo3-RS1PapBo3)^2, na.rm=T)/sum(RSPapBo3^2, na.rm=T)
	}else
...
start=list(a=1.763, b=76690, c=0.00219),control=list(maxiter=100,warnOnly=T)),silent=T)
...
mPapBo4=PapBo4$call
RSSPapBo4=sum(residuals(PapBo4)^2, na.rm=T)
TSSPapBo4=sum((trait-mean(trait))^2, na.rm=T)
PSSPapBo4=sum((predict(PapBo4)-mean(predict(PapBo4), na.rm=T))^2, na.rm=T)
...
DWPapBo4=sum((RSPapBo4-RS1PapBo4)^2, na.rm=T)/sum(RSPapBo4^2, na.rm=T)
	}else
...
start=list(a=17.25, b=0.493, c=0.0018),control=list(maxiter=100,warnOnly=T)),silent=T)
...
mPapBo6=PapBo6$call
RSSPapBo6=sum(residuals(PapBo6)^2, na.rm=T)
TSSPapBo6=sum((trait-mean(trait))^2, na.rm=T)
PSSPapBo6=sum((predict(PapBo6)-mean(predict(PapBo6), na.rm=T))^2, na.rm=T)
...
DWPapBo6=sum((RSPapBo6-RS1PapBo6)^2, na.rm=T)/sum(RSPapBo6^2, na.rm=T)
}else
...
start=list(a=18.28, b=-1.58, c=4.33),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICGS1=BIC(GS1)
mGS1=GS1$call
RSSGS1=sum(residuals(GS1)^2, na.rm=T)
TSSGS1=sum((trait-mean(trait))^2, na.rm=T)
PSSGS1=sum((predict(GS1)-mean(predict(GS1), na.rm=T))^2, na.rm=T)
FGS1=if(TSSGS1>PSSGS1){TSSGS1/PSSGS1} else {PSSGS1/TSSGS1}
RSDGS1=sqrt(RSSGS1/(n-coefGS1))
RSGS1=residuals(GS1)
RS1GS1=as.numeric(c(residuals(GS1)[-1],"NA"))
DWGS1=sum((RSGS1-RS1GS1)^2, na.rm=T)/sum(RSGS1^2, na.rm=T)
}else
...
start=list(a=17.75, b=-1.72, c=4.81, d=0.00000000007),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICGS2=BIC(GS2)
mGS2=GS2$call
RSSGS2=sum(residuals(GS2)^2, na.rm=T)
TSSGS2=sum((trait-mean(trait))^2, na.rm=T)
PSSGS2=sum((predict(GS2)-mean(predict(GS2), na.rm=T))^2, na.rm=T)
FGS2=if(TSSGS2>PSSGS2){TSSGS2/PSSGS2} else {PSSGS2/TSSGS2}
RSDGS2=sqrt(RSSGS2/(n-coefGS2))
RSGS2=residuals(GS2)
RS1GS2=as.numeric(c(residuals(GS2)[-1],"NA"))
DWGS2=sum((RSGS2-RS1GS2)^2, na.rm=T)/sum(RSGS2^2, na.rm=T)
}else
...
start=list(a=-0.08,b=3,c=3),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICLQ=BIC(LQ)
mLQ=LQ$call
RSSLQ=sum(residuals(LQ)^2, na.rm=T)
TSSLQ=sum((trait-mean(trait))^2, na.rm=T)
PSSLQ=sum((predict(LQ)-mean(predict(LQ), na.rm=T))^2, na.rm=T)
FLQ=if(TSSLQ>PSSLQ){TSSLQ/PSSLQ} else {PSSLQ/TSSLQ}
RSDLQ=sqrt(RSSLQ/(n-coefLQ))
RSLQ=residuals(LQ)
RS1LQ=as.numeric(c(residuals(LQ)[-1],"NA"))
DWLQ=sum((RSLQ-RS1LQ)^2, na.rm=T)/sum(RSLQ^2, na.rm=T)
}else
...
start=list(a=25,b=-7,c=-0.03,k=0.1),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICwil=BIC(wil)
mwil=wil$call
RSSwil=sum(residuals(wil)^2, na.rm=T)
TSSwil=sum((trait-mean(trait))^2, na.rm=T)
PSSwil=sum((predict(wil)-mean(predict(wil), na.rm=T))^2, na.rm=T)
Fwil=if(TSSwil>PSSwil){TSSwil/PSSwil} else {PSSwil/TSSwil}
RSDwil=sqrt(RSSwil/(n-coefwil))
RSwil=residuals(wil)
RS1wil=as.numeric(c(residuals(wil)[-1],"NA"))
DWwil=sum((RSwil-RS1wil)^2, na.rm=T)/sum(RSwil^2, na.rm=T)
}else
...
start=list(a=25,b=-7,c=-3,k=0.1),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICwilk=BIC(wilk)
mwilk=wilk$call
RSSwilk=sum(residuals(wilk)^2, na.rm=T)
TSSwilk=sum((trait-mean(trait))^2, na.rm=T)
PSSwilk=sum((predict(wilk)-mean(predict(wilk), na.rm=T))^2, na.rm=T)
...
DWwilk=sum((RSwilk-RS1wilk)^2, na.rm=T)/sum(RSwilk^2, na.rm=T)
}else
...
start=list(a=20,b=30,c=0.005,k=0.08),control=list(maxiter=100,warnOnly=T)),silent=T)
...
RSSwilycsml=sum(residuals(wilycsml)^2, na.rm=T)
TSSwilycsml=sum((trait-mean(trait))^2, na.rm=T)
PSSwilycsml=sum((predict(wilycsml)-mean(predict(wilycsml), na.rm=T))^2, na.rm=T)
...
DWwilycsml=sum((RSwilycsml-RS1wilycsml)^2, na.rm=T)/sum(RSwilycsml^2, na.rm=T)
	}else
...
start=list(a=26.6, b=0.0017, c=-7.68, d=0.08),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICBC=BIC(BC)
mBC=BC$call
RSSBC=sum(residuals(BC)^2, na.rm=T)
TSSBC=sum((trait-mean(trait))^2, na.rm=T)
PSSBC=sum((predict(BC)-mean(predict(BC), na.rm=T))^2, na.rm=T)
FBC=if(TSSBC>PSSBC){TSSBC/PSSBC} else {PSSBC/TSSBC}
RSDBC=sqrt(RSSBC/(n-coefBC))
RSBC=residuals(BC)
RS1BC=as.numeric(c(residuals(BC)[-1],"NA"))
DWBC=sum((RSBC-RS1BC)^2, na.rm=T)/sum(RSBC^2, na.rm=T)
	}else
...
start=list(a=19, b=0.027, c=0.08, d=0.0017),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICDJK=BIC(DJK)
mDJK=DJK$call
RSSDJK=sum(residuals(DJK)^2, na.rm=T)
TSSDJK=sum((trait-mean(trait))^2, na.rm=T)
PSSDJK=sum((predict(DJK)-mean(predict(DJK), na.rm=T))^2, na.rm=T)
FDJK=if(TSSDJK>PSSDJK){TSSDJK/PSSDJK} else {PSSDJK/TSSDJK}
RSDDJK=sqrt(RSSDJK/(n-coefDJK))
RSDJK=residuals(DJK)
RS1DJK=as.numeric(c(residuals(DJK)[-1],"NA"))
DWDJK=sum((RSDJK-RS1DJK)^2, na.rm=T)/sum(RSDJK^2, na.rm=T)
}else
...
start=list(a=3, b=-0.18, c=0.002, d=-1.4),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICMG2=BIC(MG2)
mMG2=MG2$call
RSSMG2=sum(residuals(MG2)^2, na.rm=T)
TSSMG2=sum((trait-mean(trait))^2, na.rm=T)
PSSMG2=sum((predict(MG2)-mean(predict(MG2), na.rm=T))^2, na.rm=T)
FMG2=if(TSSMG2>PSSMG2){TSSMG2/PSSMG2} else {PSSMG2/TSSMG2}
RSDMG2=sqrt(RSSMG2/(n-coefMG2))
RSMG2=residuals(MG2)
RS1MG2=as.numeric(c(residuals(MG2)[-1],"NA"))
DWMG2=sum((RSMG2-RS1MG2)^2, na.rm=T)/sum(RSMG2^2, na.rm=T)
}else
...
data=x, na.action=na.exclude, start=list(a=20.5, b=-0.38, c=-1.45, d=-0.005),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICMG4=BIC(MG4)
mMG4=MG4$call
RSSMG4=sum(residuals(MG4)^2, na.rm=T)
TSSMG4=sum((trait-mean(trait))^2, na.rm=T)
PSSMG4=sum((predict(MG4)-mean(predict(MG4), na.rm=T))^2, na.rm=T)
FMG4=if(TSSMG4>PSSMG4){TSSMG4/PSSMG4} else {PSSMG4/TSSMG4}
RSDMG4=sqrt(RSSMG4/(n-coefMG4))
RSMG4=residuals(MG4)
RS1MG4=as.numeric(c(residuals(MG4)[-1],"NA"))
DWMG4=sum((RSMG4-RS1MG4)^2, na.rm=T)/sum(RSMG4^2, na.rm=T)
}else
...
BICMG=BIC(MG)
mMG=MG$call
RSSMG=sum(residuals(MG)^2, na.rm=T)
TSSMG=sum((trait-mean(trait))^2, na.rm=T)
PSSMG=sum((predict(MG)-mean(predict(MG), na.rm=T))^2, na.rm=T)
FMG=if(TSSMG>PSSMG){TSSMG/PSSMG} else {PSSMG/TSSMG}
RSDMG=sqrt(RSSMG/(n-coefMG))
RSMG=residuals(MG)
RS1MG=as.numeric(c(residuals(MG)[-1],"NA"))
DWMG=sum((RSMG-RS1MG)^2, na.rm=T)/sum(RSMG^2, na.rm=T)
}else
...
start=list(a=15, b=-0.15, c=0.00043, d=0.0000005, f=4.05),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICKHN=BIC(KHN)
mKHN=KHN$call
RSSKHN=sum(residuals(KHN)^2, na.rm=T)
TSSKHN=sum((trait-mean(trait))^2, na.rm=T)
PSSKHN=sum((predict(KHN)-mean(predict(KHN), na.rm=T))^2, na.rm=T)
FKHN=if(TSSKHN>PSSKHN){TSSKHN/PSSKHN} else {PSSKHN/TSSKHN}
RSDKHN=sqrt(RSSKHN/(n-coefKHN))
RSKHN=residuals(KHN)
RS1KHN=as.numeric(c(residuals(KHN)[-1],"NA"))
DWKHN=sum((RSKHN-RS1KHN)^2, na.rm=T)/sum(RSKHN^2, na.rm=T)
}else
...
start=list(a=19, b=-5, c=0.003, d=5.3, e=-1.1),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICAS=BIC(AS)
mAS=AS$call
RSSAS=sum(residuals(AS)^2, na.rm=T)
TSSAS=sum((trait-mean(trait))^2, na.rm=T)
PSSAS=sum((predict(AS)-mean(predict(AS), na.rm=T))^2, na.rm=T)
FAS=if(TSSAS>PSSAS){TSSAS/PSSAS} else {PSSAS/TSSAS}
RSDAS=sqrt(RSSAS/(n-coefAS))
RSAS=residuals(AS)
RS1AS=as.numeric(c(residuals(AS)[-1],"NA"))
DWAS=sum((RSAS-RS1AS)^2, na.rm=T)/sum(RSAS^2, na.rm=T)
}else
...
start=list(a=15, b=-0.08, c=8, d=-3, e=-0.00004),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICFRP=BIC(FRP)
mFRP=FRP$call
RSSFRP=sum(residuals(FRP)^2, na.rm=T)
TSSFRP=sum((trait-mean(trait))^2, na.rm=T)
PSSFRP=sum((predict(FRP)-mean(predict(FRP), na.rm=T))^2, na.rm=T)
FFRP=if(TSSFRP>PSSFRP){TSSFRP/PSSFRP} else {PSSFRP/TSSFRP}
RSDFRP=sqrt(RSSFRP/(n-coefFRP))
RSFRP=residuals(FRP)
RS1FRP=as.numeric(c(residuals(FRP)[-1],"NA"))
DWFRP=sum((RSFRP-RS1FRP)^2, na.rm=T)/sum(RSFRP^2, na.rm=T)
}else
...
start=list(a=25, b=-0.001),control=list(maxiter=100,warnOnly=T)),silent=T)
...
mPTmult=PTmult$call
RSSPTmult=sum(residuals(PTmult)^2, na.rm=T)
TSSPTmult=sum((trait-mean(trait))^2, na.rm=T)
PSSPTmult=sum((predict(PTmult)-mean(predict(PTmult), na.rm=T))^2, na.rm=T)
...
DWPTmult=sum((RSPTmult-RS1PTmult)^2, na.rm=T)/sum(RSPTmult^2, na.rm=T)
}else
...
start=list(a=25, b=0.36, c=0.13, d=-0.001),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICPTmod=BIC(PTmod)
mPTmod=PTmod$call
RSSPTmod=sum(residuals(PTmod)^2, na.rm=T)
TSSPTmod=sum((trait-mean(trait))^2, na.rm=T)
PSSPTmod=sum((predict(PTmod)-mean(predict(PTmod), na.rm=T))^2, na.rm=T)
...
DWPTmod=sum((RSPTmod-RS1PTmod)^2, na.rm=T)/sum(RSPTmod^2, na.rm=T)
}else
...
start=list(a=7703, b=0.002, c=277),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICMonoG=BIC(MonoG)
mMonoG=MonoG$call
RSSMonoG=sum(residuals(MonoG)^2, na.rm=T)
TSSMonoG=sum((trait-mean(trait))^2, na.rm=T)
PSSMonoG=sum((predict(MonoG)-mean(predict(MonoG), na.rm=T))^2, na.rm=T)
...
DWMonoG=sum((RSMonoG-RS1MonoG)^2, na.rm=T)/sum(RSMonoG^2, na.rm=T)
}else
...
start=list(a=24.5707, b=0.6292, c=1.9977, k=0.1978),control=list(maxiter=100,warnOnly=T)),silent=T)
...
RSSMonoGpw=sum(residuals(MonoGpw)^2, na.rm=T)
TSSMonoGpw=sum((trait-mean(trait))^2, na.rm=T)
PSSMonoGpw=sum((predict(MonoGpw)-mean(predict(MonoGpw), na.rm=T))^2, na.rm=T)
...
DWMonoGpw=sum((RSMonoGpw-RS1MonoGpw)^2, na.rm=T)/sum(RSMonoGpw^2, na.rm=T)
}else
...
start=list(a=353.8,b=0.016,c=36.3, d=7371, f=0.003, g=113.5),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICDiG=BIC(DiG)
mDiG=DiG$call
RSSDiG=sum(residuals(DiG)^2, na.rm=T)
TSSDiG=sum((trait-mean(trait))^2, na.rm=T)
PSSDiG=sum((predict(DiG)-mean(predict(DiG), na.rm=T))^2, na.rm=T)
FDiG=if(TSSDiG>PSSDiG){TSSDiG/PSSDiG} else {PSSDiG/TSSDiG}
RSDDiG=sqrt(RSSDiG/(n-coefDiG))
RSDiG=residuals(DiG)
RS1DiG=as.numeric(c(residuals(DiG)[-1],"NA"))
DWDiG=sum((RSDiG-RS1DiG)^2, na.rm=T)/sum(RSDiG^2, na.rm=T)
}else
...
start=list(a=15.99,b=0.3351,c=4.693, d=8687, f=0.00226, g=80.33, k=0.435),control=list(maxiter=100,warnOnly=T)),silent=T)
...
BICDiGpw=BIC(DiGpw)
mDiGpw=DiGpw$call
RSSDiGpw=sum(residuals(DiGpw)^2, na.rm=T)
TSSDiGpw=sum((trait-mean(trait))^2, na.rm=T)
PSSDiGpw=sum((predict(DiGpw)-mean(predict(DiGpw), na.rm=T))^2, na.rm=T)
...
DWDiGpw=sum((RSDiGpw-RS1DiGpw)^2, na.rm=T)/sum(RSDiGpw^2, na.rm=T)
}else
...
legpol3=try(lm(trait ~ leg3, data=x, na.action=na.exclude),silent=T)
...
RSSlegpol3=sum(residuals(legpol3)^2, na.rm=T)
TSSlegpol3=sum((trait-mean(trait))^2, na.rm=T)
PSSlegpol3=sum((predict(legpol3)-mean(predict(legpol3), na.rm=T))^2, na.rm=T)
...
DWlegpol3=sum((RSlegpol3-RS1legpol3)^2, na.rm=T)/sum(RSlegpol3^2, na.rm=T)
}else
...
legpol4=try(lm(trait ~ leg4, data=x, na.action=na.exclude),silent=T)
...
RSSlegpol4=sum(residuals(legpol4)^2, na.rm=T)
TSSlegpol4=sum((trait-mean(trait))^2, na.rm=T)
PSSlegpol4=sum((predict(legpol4)-mean(predict(legpol4), na.rm=T))^2, na.rm=T)
...
DWlegpol4=sum((RSlegpol4-RS1legpol4)^2, na.rm=T)/sum(RSlegpol4^2, na.rm=T)
}else
...
start=list(a=-0.8, b=-0.6,c=0.1,d=25.7, k=0.002)),silent=T)
...
RSSlegpolWil=sum(residuals(legpolWil)^2, na.rm=T)
TSSlegpolWil=sum((trait-mean(trait))^2, na.rm=T)
PSSlegpolWil=sum((predict(legpolWil)-mean(predict(legpolWil), na.rm=T))^2, na.rm=T)
...
DWlegpolWil=sum((RSlegpolWil-RS1legpolWil)^2, na.rm=T)/sum(RSlegpolWil^2, na.rm=T)
}else
...
cubsplin3=try(lm(trait ~ ns(dim, df = 4), data = x, na.action=na.exclude),silent=T)
...
RSScubsplin3=sum(residuals(cubsplin3)^2, na.rm=T)
TSScubsplin3=sum((trait-mean(trait))^2, na.rm=T)
PSScubsplin3=sum((predict(cubsplin3)-mean(predict(cubsplin3), na.rm=T))^2, na.rm=T)
...
DWcubsplin3=sum((RScubsplin3-RS1cubsplin3)^2, na.rm=T)/sum(RScubsplin3^2, na.rm=T)
}else
...
cubsplin4=try(lm(trait ~ ns(dim, df = 5), data = x, na.action=na.exclude),silent=T)
...
RSScubsplin4=sum(residuals(cubsplin4)^2, na.rm=T)
TSScubsplin4=sum((trait-mean(trait))^2, na.rm=T)
PSScubsplin4=sum((predict(cubsplin4)-mean(predict(cubsplin4), na.rm=T))^2, na.rm=T)
...
DWcubsplin4=sum((RScubsplin4-RS1cubsplin4)^2, na.rm=T)/sum(RScubsplin4^2, na.rm=T)
}else
...
cubsplin5=try(lm(trait ~ ns(dim, df = 6), data = x, na.action=na.exclude),silent=T)
...
RSScubsplin5=sum(residuals(cubsplin5)^2, na.rm=T)
TSScubsplin5=sum((trait-mean(trait))^2, na.rm=T)
PSScubsplin5=sum((predict(cubsplin5)-mean(predict(cubsplin5), na.rm=T))^2, na.rm=T)
...
DWcubsplin5=sum((RScubsplin5-RS1cubsplin5)^2, na.rm=T)/sum(RScubsplin5^2, na.rm=T)
}else
...
cubsplindef=try(lm(trait ~ ns(dim, knots =c(49,78,112,157,210)), data = x, na.action=na.exclude),silent=T)
...
RSScubsplindef=sum(residuals(cubsplindef)^2, na.rm=T)
TSScubsplindef=sum((trait-mean(trait))^2, na.rm=T)
PSScubsplindef=sum((predict(cubsplindef)-mean(predict(cubsplindef), na.rm=T))^2, na.rm=T)
...
DWcubsplindef=sum((RScubsplindef-RS1cubsplindef)^2, na.rm=T)/sum(RScubsplindef^2, na.rm=T)
}else
...
new=matrix("",45,8)
options(warn=-1)
new[,1][selcri2[,1]==max(as.numeric(selcri2[,1]),na.rm=T)]=max(as.numeric(selcri2[,1]),na.rm=T) #R2
new[,2][selcri2[,2]==max(as.numeric(selcri2[,2]),na.rm=T)]=max(as.numeric(selcri2[,2]),na.rm=T) #R2adj
new[,3][selcri2[,3]==min(as.numeric(selcri2[,3]),na.rm=T)]=min(as.numeric(selcri2[,3]),na.rm=T) #RSE
new[,4][selcri2[,4]==max(as.numeric(selcri2[,4]),na.rm=T)]=max(as.numeric(selcri2[,4]),na.rm=T) #logL
new[,5][selcri2[,5]==min(as.numeric(selcri2[,5]),na.rm=T)]=min(as.numeric(selcri2[,5]),na.rm=T) #AIC
new[,6][selcri2[,6]==min(as.numeric(selcri2[,6]),na.rm=T)]=min(as.numeric(selcri2[,6]),na.rm=T) #AICC
new[,7][selcri2[,7]==min(as.numeric(selcri2[,7]),na.rm=T)]=min(as.numeric(selcri2[,7]),na.rm=T) #BIC
...
R2=c(rownames(selcri2)[selcri2[,1]==max(as.numeric(selcri2[,1]),na.rm=T)],max(as.numeric(selcri2[,1]),na.rm=T))
R2adj=c(rownames(selcri2)[selcri2[,2]==max(as.numeric(selcri2[,2]),na.rm=T)],max(as.numeric(selcri2[,2]),na.rm=T))
RSE=c(rownames(selcri2)[selcri2[,3]==min(as.numeric(selcri2[,3]),na.rm=T)],min(as.numeric(selcri2[,3]),na.rm=T))
logL=c(rownames(selcri2)[selcri2[,4]==max(as.numeric(selcri2[,4]),na.rm=T)],max(as.numeric(selcri2[,4]),na.rm=T))
AIC=c(rownames(selcri2)[selcri2[,5]==min(as.numeric(selcri2[,5]),na.rm=T)],min(as.numeric(selcri2[,5]),na.rm=T))
AICC=c(rownames(selcri2)[selcri2[,6]==min(as.numeric(selcri2[,6]),na.rm=T)],min(as.numeric(selcri2[,6]),na.rm=T))
BIC=c(rownames(selcri2)[selcri2[,7]==min(as.numeric(selcri2[,7]),na.rm=T)],min(as.numeric(selcri2[,7]),na.rm=T))
...
R2model=c(
	...
	R2=(if(critbest[1,1]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	R2=(if(critbest[1,1]=="(30) Diphasic Grossman")mDiG),
	R2=(if(critbest[1,1]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	R2=(if(critbest[1,1]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	R2=(if(critbest[1,1]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	R2=(if(critbest[1,1]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
R2adjmodel=c(
	...
	R2adj=(if(critbest[1,2]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	R2adj=(if(critbest[1,2]=="(30) Diphasic Grossman")mDiG),
	R2adj=(if(critbest[1,2]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	R2adj=(if(critbest[1,2]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	R2adj=(if(critbest[1,2]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	R2adj=(if(critbest[1,2]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
RSEmodel=c(
	...
	RSE=(if(critbest[1,3]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	RSE=(if(critbest[1,3]=="(30) Diphasic Grossman")mDiG),
	RSE=(if(critbest[1,3]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	RSE=(if(critbest[1,3]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	RSE=(if(critbest[1,3]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	RSE=(if(critbest[1,3]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
logLmodel=c(
	...
	logL=(if(critbest[1,4]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	logL=(if(critbest[1,4]=="(30) Diphasic Grossman")mDiG),
	logL=(if(critbest[1,4]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	logL=(if(critbest[1,4]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	logL=(if(critbest[1,4]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	logL=(if(critbest[1,4]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
AICmodel=c(
	...
	AIC=(if(critbest[1,5]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	AIC=(if(critbest[1,5]=="(30) Diphasic Grossman")mDiG),
	AIC=(if(critbest[1,5]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	AIC=(if(critbest[1,5]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	AIC=(if(critbest[1,5]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	AIC=(if(critbest[1,5]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
AICCmodel=c(
	...
	AICC=(if(critbest[1,6]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	AICC=(if(critbest[1,6]=="(30) Diphasic Grossman")mDiG),
	AICC=(if(critbest[1,6]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	AICC=(if(critbest[1,6]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	AICC=(if(critbest[1,6]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	AICC=(if(critbest[1,6]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
BICmodel=c(
	...
	BIC=(if(critbest[1,7]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	BIC=(if(critbest[1,7]=="(30) Diphasic Grossman")mDiG),
	BIC=(if(critbest[1,7]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	BIC=(if(critbest[1,7]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	BIC=(if(critbest[1,7]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	BIC=(if(critbest[1,7]=="(34) Legendre + Wilmink (Lopez)")mlegpolWil),
...
DWmodel=c(
	...
	DW=(if(critbest[1,8]=="(29) Monophasic Grossman power (Weigel, Sherchand)")mMonoGpw),
	DW=(if(critbest[1,8]=="(30) Diphasic Grossman")mDiG),
	DW=(if(critbest[1,8]=="(31) Diphasic Grossman power (Weigel, Sherchand)")mDiGpw),
	DW=(if(critbest[1,8]=="(32) Legendre Polynomial 3th (Kirkpatrick)")mlegpol3),
	DW=(if(critbest[1,8]=="(33) Legendre Polynomial 4th (Kirkpatrick)")mlegpol4),
	DW=(if(critbest[1,8]=="(34) Legendre +  Wilmink (Lopez)")mlegpolWil),
